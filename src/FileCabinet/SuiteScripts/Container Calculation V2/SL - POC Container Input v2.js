...
/**
 * Container Input Processing Suitelet
 * @summary Container Input Data Processor
 * @author Phong <phong@teibto.com>
 * @version 1.0
 */

define(['N/error', 'N/file', 'N/record', 'N/runtime', 'N/search', 'N/cache', 'N/ui/serverWidget', 'N/redirect', 'N/https',  'N/url', 'N/log'],
    function(error, file, record, runtime, search, cache, serverWidget, redirect, https, url, log) {
        ...
        function onRequest(context) {
            ...
            if (request.method === 'POST') {
                ...
                log.debug('jsonData.items', jsonData.items);
                if (jsonData.items) {
                    ...
                    for (var i = 0; i < jsonData.items.length; i++) {
                        ...
                        var containerPalletResults = containerPalletByCustomerSearch.run().getRange({
                            start: 0,
                            end: 1
                        }) || [];
                        ...
                        specialConditionSearch.run().each(function (result) {
                            ...
                            if (palletType) {
                                palletChecked = true;
                                var palletTypeArr = palletType.split(',').map(function (v) {
                                    return v.trim();
                                });
                            ...
                            if (scaItemSubtype) {
                                subtypeChecked = true;
                                var scaItemSubtypeArr = scaItemSubtype.split(',').map(function (v) {
                                    return v.trim();
                                });
                            ...
                            specialConditionResult.forEach(function (rec) {
                                ...
                                var scData = {
                                    ...
                                    scScaitemsubtype: rec.getValue('custrecord_criteria_scaitemsubtype')
                                        ? rec.getValue('custrecord_criteria_scaitemsubtype').toString().split(',').map(function (v) {
                                            return v.trim();
                                        })
                                        : [],
                                    ...
                                    scPapersize: rec.getValue('custrecord_criteria_papersize')
                                        ? rec.getValue('custrecord_criteria_papersize').toString().split(',').map(function (v) {
                                            return v.trim();
                                        })
                                        : [],
                                    ...
                                    scLayer: rec.getValue('custrecord_criteria_layer')
                                        ? rec.getValue('custrecord_criteria_layer').toString().split(',').map(function (v) {
                                            return v.trim();
                                        })
                                        : [],
                        ...
                        if (mixed === true) {
                            ...
                            if (searchResult.length > 0) {
                                searchResult.forEach(function (result) {
                                    ...
                                    if (unitTypeId) {
                                        var unitTypeRecord = record.load({
                                        ...
                                        for (var j = 0; j < lineCount; j++) {
                                            var abbreviation = unitTypeRecord.getSublistValue({
                                                sublistId: 'uom',
                                                fieldId: 'abbreviation',
                                                line: j
                                            });
                                            var baseUnit = unitTypeRecord.getSublistValue({
                                                sublistId: 'uom',
                                                fieldId: 'baseunit',
                                                line: j
                                            });
                                            var conversionRate = unitTypeRecord.getSublistValue({
                                                sublistId: 'uom',
                            ...
                            if (productTypeName && productTypeName.toLowerCase().includes('corrugating medium paper (roll)')) {
